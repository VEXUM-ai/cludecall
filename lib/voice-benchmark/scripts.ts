...
export const VOICE_SCRIPT_CASES: VoiceScriptCase[] = [
  ...
  {
    id: "patient_name_yomi_prompt",
    title: "氏名の読み確認",
    category: "guardrail",
    expectedText: "お名前の読み方を確認させてください。",
    expectedAlternatives: ["お名前の読み方を教えてください。"],
    mustContain: ["読み方"],
    shouldNotContain: ["ひらがなで", "漢字で", "カタカナで"],
    keyterms: [TSUKUDA_NAME],
    notes: "音声会話では表記種別を要求せず、読み方だけを聞けているかを見る。",
  },
  {
    id: "patient_name_readback",
    title: "氏名の復唱",
    category: "pronunciation",
    expectedText: TSUKUDA_NAME_WITH_HONORIFIC,
    expectedAlternatives: [`${TSUKUDA_NAME}さまですね。`],
    mustContain: ["つくだ", "とうじ"],
    keyterms: [TSUKUDA_NAME],
    notes: "名前の読みを自然に復唱できるかを確認する。",
  },
  {
...
export const VOICE_CONVERSATION_SCENARIOS: VoiceConversationScenario[] = [
  ...
  {
    id: "name_reading_self_correction",
    title: "氏名の聞き間違い訂正",
    channel: "phone",
    lineCondition: "hesitant",
    userPrompt:
      "親知らずの相談です。名前はつくだです。あ、すみません、つくだとうじです。必要なら読み方は確認していいですが、表記ではなく読み方だけ聞いてください。",
    goals: ["氏名の訂正追従", "読み確認の自然さ", "禁止文言回避"],
    mustInclude: ["つくだとうじ", "読み方"],
    shouldNotSay: ["ひらがなで", "漢字で", "カタカナで"],
    notes: "今回の実バグ対策。表記種別要求と名前確認ループを避ける。",
  },
  {
